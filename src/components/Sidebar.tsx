...
const menuItems = [
  ...
  { path: "/uploads", label: "Uploads", activePaths: ["/uploads"] },
  { path: "/about", label: "About", activePaths: ["/about"] },
];
          ...
          {menuItems.map((item) => {
            ...
            return (
              <NavLink
                key={item.path}
                to={item.path}
                className={`nav-link my-1 py-3 px-4 text-white rounded-2 move-up-hover ${
                   isActive ? "bg-primary active-item" : "hover-effect"
                }`}
                style={{ 
